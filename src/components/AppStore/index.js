...
class AppStore extends Component {
  ...
  searchFilter = () => {
    ...
    const search = appsList.filter(
      items =>
        items.category === ActiveTabId &&
        items.appName.toLowerCase().includes(searchinput.toLowerCase()),
    )
    return search
  }
  ...
  render() {
    ...
    const searchResults = this.searchFilter()
    console.log(searchResults)
    return (
      <div className="bg_container">
        <div className="card_container">
          <h1 className="heading_app">App Store</h1>
          ...
          <ul className="bodyContainer">
            {searchResults.map(eachItem => (
              <AppItem tabItem={eachItem} key={eachItem.appId} />
            ))}
          </ul>
        </div>
      </div>
